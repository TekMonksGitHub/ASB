...
exports.createHTTPServer = listener => {
    ...
    if(listener.flow.listener.ssl){
        const httpsServerOptions = {
            key: fs.readFileSync(listener.flow.listener.sslKeyFile),
            cert: fs.readFileSync(listener.flow.listener.sslCertFile)
        }; server = https.createServer(httpsServerOptions).listen(port, host);
    }
    else server = http.createServer().listen(port, host);
    
    initServer(server, listener);
    ESB.env.HTTPSERVERS[`${host}:${port}`] = server;

    const serverType = server instanceof http.Server? "HTTP" : "HTTPS";
    LOG.info(`[HTTP_SERVER_FACTORY] ${serverType} server listening on ${host}:${port}`);
