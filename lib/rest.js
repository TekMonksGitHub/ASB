...
function patch(host, port, path, headers, req, callback) {
    let jsonStr = typeof (req) == "object" ? JSON.stringify(req) : req;
    ...
    doCall(jsonStr, optionspatch, false, callback);
}

function patchHttps(host, port, path, headers, req, callback) {
    let jsonStr = typeof (req) == "object" ? JSON.stringify(req) : req;
    ...
    doCall(jsonStr, optionspatch, true, callback);
}
...
function doCall(reqStr, options, secure, callback) {
    ...
    let req = caller.request(options, (res) => {
        ...
        res.on("end", function() {
            if(responseString != "") try {responseString = JSON.parse(JSON.stringify(responseString))}catch (e) {callback("Bad JSON Response", null)}
            callback(null, responseString, this);
        });
    });
