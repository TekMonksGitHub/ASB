...
const fs = require("fs");
const Timer = require(`${CONSTANTS.LIBDIR}/Timer.js`).Timer;

class FileWriter {
    constructor(path, writeCloseTimeout, encoding) {
        this.path = path;     
    ...
    _writeToFile(data, callback) {
        ...
        fs.writeFile(this._env.fd, data, this.encoding, e => {
            this._env.writesPending--;
            this._env.timer.addTime(this.writeCloseTimeout);
            callback(e);
        });
    }
    ...
    writeFile(data, callback) {
        ...
        fs.open(this.path, "a", (err, fd) => {
            ...
            env.timer = new Timer(this.writeCloseTimeout, _ => { 
                if (env.writesPending) 
                    env.timer.addTime(this.writeCloseTimeout); // don't close if we have pending writes
                else {
