...
const INDEPENDENT_EXECUTION = global.CONSTANTS?.ESBCONF == undefined;    // support independent execution
let CONSTANTS, LOG;
...
function post(host, port, path, headers, req, timeout, sslObj, callback) {
    headers = headers||{}; const body = req; _addHeaders(headers, body);
    const optionspost = { host, port, path, method: 'POST', headers };
    const result = _doCall(body, optionspost, false, sslObj, timeout); if (!callback) return result;
...
function postHttps(host, port, path, headers, req, timeout, sslObj, callback) {
    headers = headers||{}; const body = req; _addHeaders(headers, body);
    const optionspost = { host, port, path, method: 'POST', headers };
    const result = _doCall(body, optionspost, true, sslObj, timeout); if (!callback) return result;
    result.then(({ data, status, resHeaders, error }) => callback(error, data, status, resHeaders)).catch((error) => callback(error, null));
}

function put(host, port, path, headers, req, timeout, sslObj, callback) {
    headers = headers||{}; const body = req; _addHeaders(headers, body);
    const optionsput = { host, port, path, method: 'PUT', headers };
    const result = _doCall(body, optionsput, false, sslObj, timeout); if (!callback) return result;
    result.then(({ data, status, resHeaders, error }) => callback(error, data, status, resHeaders)).catch((error) => callback(error, null));
}
...
function putHttps(host, port, path, headers, req, timeout, sslObj, callback) {
    headers = headers||{}; const body = req; _addHeaders(headers, body);
    const optionsput = { host, port, path, method: 'PUT', headers };
    const result = _doCall(body, optionsput, true, sslObj, timeout); if (!callback) return result;
    result.then(({ data, status, resHeaders, error }) => callback(error, data, status, resHeaders)).catch((error) => callback(error, null));
}

function get(host, port, path, headers, req, timeout, sslObj, callback) {
    ...
    const result = _doCall(null, optionsget, false, sslObj, timeout); if (!callback) return result;
    result.then(({ data, status, resHeaders, error }) => callback(error, data, status, resHeaders)).catch((error) => callback(error, null));
}
...
function getHttps(host, port, path, headers = {}, req, timeout, sslObj, callback) {
    ...
    if (req && typeof req == "object") req = querystring.stringify(req); if (req && req.trim() !== "") path += `?${req}`; 
    const optionsget = { host, port, path, method: 'GET', headers };
    const result = _doCall(null, optionsget, true, sslObj, timeout); if (!callback) return result;
    result.then(({ data, status, resHeaders, error }) => callback(error, data, status, resHeaders)).catch((error) => callback(error, null));
}

function deleteHttp(host, port, path, headers, req, timeout, sslObj, callback) {
    headers = headers||{}; _addHeaders(headers, null);
    if (req && typeof req == "object") req = querystring.stringify(req); if (req && req.trim() !== "") path += `?${req}`; 
    const optionsdelete = { host, port, path, method: 'DELETE', headers };
    const result = _doCall(null, optionsdelete, false, sslObj, timeout); if (!callback) return result;
    result.then(({ data, status, resHeaders, error }) => callback(error, data, status, resHeaders)).catch((error) => callback(error, null));
}
...
function deleteHttps(host, port, path, headers = {}, req, timeout, sslObj, callback) {
    ...
    if (req && typeof req == "object") req = querystring.stringify(req); if (req && req.trim() !== "") path += `?${req}`; 
    const optionsdelete = { host, port, path, method: 'DELETE', headers };
    const result = _doCall(null, optionsdelete, true, sslObj, timeout); if (!callback) return result;
    result.then(({ data, status, resHeaders, error }) => callback(error, data, status, resHeaders)).catch((error) => callback(error, null));
}
...
async function fetch(url, options={}, redirected=false) {    // somewhat fetch compatible API
    ...
    if (options.undici && (!_haveUndiciModule())) ASBLOG.warn(`HTTP client told to use Undici in fetch for URL ${url}. But Undici NPM is not installed. Falling back to the native HTTP clients.`);
    const { error, data, status, resHeaders } = undici ? await _undiciRequest(url, options) : 
        await module.exports[method](urlObj.hostname, port, totalPath, headers, body, undefined, sslOptions);
    ...
    if (status == 301 || status == 302 || status == 303 || status == 307 || status == 308 && 
        ...
        else ASBLOG.error(`URL ${url} sent a redirect with no location specified (location header not found).`);
    }
...
async function _undiciRequest(url, options) {
    ASBLOG.info(`httpClient connecting to URL ${url} via HTTP1.1 using Undici.`);
    ...
    if (resHeaders["content-encoding"] == "gzip") try {dataBuffer = await gunzipAsync(dataBuffer)} catch (err) {
        ASBLOG.error(`Gunzip decompression error for Undici request to the URL ${url}.`);
...
function _doCall(reqStr, options, secure, sslObj, timeout) {
    return new Promise(async (resolve, reject) => {
        ...
        const timeoutID = timeout?setTimeout(_=>reject("Timeout error"), timeout):undefined;

        if (secure && (!sslObj?._org_monkshu_httpclient_forceHTTP1)) { // for http2 case
            ASBLOG.info(`httpClient connecting to URL ${options.host}:${options.port}/${options.path} via HTTP2.`);
            ...
            ASBLOG.info(`httpClient connecting to URL ${options.host}:${options.port}/${options.path} via HTTP1.`);
...
async function _addSecureOptions(options, sslObj) {
    ...
    if (sslObj.rejectUnauthorized !== undefined) {
        options.rejectUnauthorized = sslObj.rejectUnauthorized;
        process.env["NODE_TLS_REJECT_UNAUTHORIZED"] = 0;    // insecure, yes but sometimes only way!
    }
}
