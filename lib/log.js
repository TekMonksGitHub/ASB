...
let origLog; let oldStdoutWrite; let oldStderrWrite;
let filewriter;
...
Logger.prototype.console = function(s) {
	origLog(s?s.trim():s);						// send to console or debug console, trimmed
	oldStdoutWrite.call(process.stdout, s);		// send to process' STDOUT
}
...
Logger.prototype.overrideConsole = function() {
	...
	process.on('uncaughtException', function(err) {
		...
		oldStderrWrite.call(process.stderr, "EXIT ON CRITICAL ERROR!!! Check Logs.");
		process.exit(1);
	});
};
