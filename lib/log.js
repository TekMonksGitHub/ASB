...
const log_conf = require(ASB_CONSTANTS.LOGSCONF);
let origLog;

function initGlobalLoggerSync(logName) {
	/* create the logger */
	if (!fs.existsSync(ASB_CONSTANTS.LOGDIR)) fs.mkdirSync(ASB_CONSTANTS.LOGDIR);
	let filewriter = 
		require(`${ASB_CONSTANTS.LIBDIR}/FileWriter.js`).createFileWriter(logName,log_conf.closeTimeOut,"utf8");
...
Logger.prototype.truncate = function(s) {return s.length > ASB_CONSTANTS.MAX_LOG ? s.substring(0, ASB_CONSTANTS.MAX_LOG) : s;}
