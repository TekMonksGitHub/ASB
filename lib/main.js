...
global.ESB = global.ESB || {_running_flows: {}, "env":{}, "launchNewFlowInstance": launchNewFlowInstance, conf};       // The global ESB namespace
    ...
    try {
        ...
        if (!noflows) for (const filename of fs.readdirSync(ASBCONSTANTS.FLOWSDIR)) try {  // no flows means flows will be added manually later
            if ((!filename.toLowerCase().endsWith("json")) && (!filename.toLowerCase().endsWith("yaml"))) {
                let errMsg = `Bad flow file, skipping: ${filename}`; console.log(errMsg); ASBLOG.error(errMsg, true); 
                continue;
            }
            const flowRaw = filename.toLowerCase().endsWith("json") ? JSON.parse(fs.readFileSync(`${ASBCONSTANTS.FLOWSDIR}/${filename}`, "UTF-8")) :
                yaml.parse(fs.readFileSync(`${ASBCONSTANTS.FLOWSDIR}/${filename}`, "UTF-8"));
...
function initFlow(flowThis) {
    if (!flowThis.env) flowThis.env = {};   // in-proc flows may already have an environment coming in
...
function launchNewFlowInstance(flow) {
    ...
    for (const route of routeNames) flow[route].flow = flow;
    runFlow(flow, routeNames);
}

function runFlow(flow, routeNames) {
    stopFlow(flow); // stop the currently running instance, if any
    ...
    global.ESB._running_flows[flow.flow.name] = setIntervalImmediately(flowRun, flow.flow.cpuInterval||conf.cpuInterval); // run at given frequency and first one immediately
}

function stopFlow(flow) {
    if (global.ESB._running_flows[flow.flow.name]) { 
        clearInterval(global.ESB._running_flows[flow.flow.name]);  
        delete global.ESB._running_flows[flow.flow.name]
    }
}
...
function _isBooleanDependencyExpression(expression) {
    ...
    if (!testExpression) return false;
...
function _checkBooleanDependencyExpressionIsSatisfied(expression, routesDone, flow) {
    ...
    let evalCode = ""; for (const routeNode of Object.keys(flow)) {
        ...
        evalCode += `const ${`${routeNode}.error`.replaceAll(".","_")} = ${routesDone.includes(`${routeNode}.error`)?"true":"false"};\n`;
    }
