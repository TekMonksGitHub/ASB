...
global.ASB_CONSTANTS = require(__dirname + "/constants.js");
const fs = require("fs");
const conf = require(ASB_CONSTANTS.ESBCONF);
...
function bootstrap() {
    ...
    require(ASB_CONSTANTS.LIBDIR+"/log.js").initGlobalLoggerSync(ASB_CONSTANTS.LOGMAIN);

    /* Setup message factory */
    global.MESSAGE_FACTORY = require(ASB_CONSTANTS.LIBDIR+"/messageFactory.js");

    /* Run the flows */
    try {
        const flowsToRun = [];
        fs.readdirSync(ASB_CONSTANTS.FLOWSDIR).forEach( filename => { 
            try {
                let flowThis = JSON.parse(fs.readFileSync(`${ASB_CONSTANTS.FLOWSDIR}/${filename}`, "UTF-8"));
                flowThis.env = {};
...
function runFlowRoute(flow, route, routeObj, messageContainer, message) {
    ...
    try {
        ...
        require(`${ASB_CONSTANTS.ROOTDIR}/${dirPath}/${routeObj.type}.js`).start(route, _expandRouteProps(routeObj, flow, message), messageContainer, message);
