...
function bootstrap(noflows) {
    ...
    try {
        const flowsToRun = [];
        if (!noflows) for (const filename of fs.readdirSync(CONSTANTS.FLOWSDIR)) try {  // no flows means flows will be added manually later
            const flowRaw = JSON.parse(fs.readFileSync(`${CONSTANTS.FLOWSDIR}/${filename}`, "UTF-8"));
