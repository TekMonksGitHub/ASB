...
exports.start = (routeName, sftpdownload, _messageContainer, message) => {
    ...
    const routeDone = (err, connection, content) => {
        ...
        _log(ASBLOG.debug,"Disconnected from "+sftpdownload.host,connection);
    ...
    if (!localpath) {
        ...
        _log(ASBLOG.info,`Message with timestamp ${message.timestamp} is being downloaded via temp local file ${localpath}`,{__asb_id:timestamp});
    }
    ...
    sshConnection.on("ready", _ => {
        ...
        sshConnection.sftp((err, sftpConnection) => {
            ...
            _log(ASBLOG.debug,`Downloading ${remotePath}->${localpath} from ${sftpdownload.host}`,sshConnection);
