...
exports.start = (routeName, filereader, _messageContainer, message) => {
    ...
    if(!ESB.env.totalMemory) ESB.env.totalMemory = 0;
    if(ESB.env.totalMemory > filereader.totalMax)return;
    ...
    let handleReadResult = (e, data) => {
        if (e) handleError(`Read error: ${e}`); else {
            if(ESB.env.totalMemory > filereader.totalMax) {message.env[routeName].isBeingProcessed = false; filereader.flow.env.fileNum--; return;}
            ESB.env.totalMemory+=data.length/1000000;
