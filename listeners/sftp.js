...
const log = (logFunction, message, sshConnection) => logFunction.bind(LOG)(`[SFTP_LISTENER] [CONNECTION: ${sshConnection.__asb_id}] ${message}`);
...
exports.start = (routeName, sftp, messageContainer, _message) => {
    ...
    const timestamp = Date.now(); log(LOG.debug,`Watching file/s: ${sftp.path} on ${sftp.host}`,{__asb_id: timestamp});
    ...
    const countFilesProcessed = (count, connection, totalToProcess) => {
        count++; if (count == totalToProcess) { connection.end(); log(LOG.debug,"Disconnected from "+sftp.host, sshConnection); 
    ...
    sshConnection.on("ready", _ => {
        log(LOG.debug,"sftp client is connected",sshConnection);
        sshConnection.sftp((err, sftpConnection) => {
            if (err) {  // ssh connection error
                log(LOG.error,"sftp error: "+err, sshConnection); log(LOG.debug,"Disconnected from "+sftp.host, sshConnection);
            ...
            sftpConnection.readdir(path.dirname(sftp.path), (err, files) => {
                if (err || files.length == 0) {  // sftp readdir error
                    if (err) log(LOG.error,"sftp error: "+err,sshConnection); 
                    log(LOG.debug, "Disconnected from "+sftp.host, sshConnection);
                    sshConnection.end(); sftp.flow.env[routeName] = {"busy":false}; 
                    return;
                }
        ...
        log(LOG.error,"ssh connect error: "+err, sshConnection); 
        sshConnection.end(); sftp.flow.env[routeName] = {"busy":false}; 
        return;
...
function processFile(file, routeName, listener, messageContainer, sftpConnection, sshConnection, callback) {
    log(LOG.info,`Detected: ${file}`,sshConnection); 
    ...
    if (!message) {log(LOG.error,"Message creation error, throttling listener.",sshConnection); callback("Throttling error"); return;}
    sftpConnection.fastGet(file, newPath, err => {
        if (err) {log(LOG.error,`Error downloading: ${err}`,sshConnection); callback(err); return;}
        sftpConnection.rename(file, newRemotePath, err => {
            if (err) {log(LOG.error,`Error renaming remote file: ${err}`,sshConnection); callback(err); return;}
            message.env.filepath = newPath;
            message.addRouteDone(routeName);
            messageContainer.add(message);
            log(LOG.info,`Injected message with timestamp: ${message.timestamp}`,sshConnection); 
            callback();
        });
    });
}