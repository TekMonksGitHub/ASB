...
exports.start = (routeName, sftp, messageContainer, _message) => {
    ...
    const timestamp = Date.now(); _log(ASBLOG.debug,`Watching file/s: ${sftp.path} on ${sftp.host}`,{__asb_id: timestamp});
    ...
    const countFilesProcessed = (count, connection, totalToProcess) => {
        count++; if (count == totalToProcess) { connection.end(); _log(ASBLOG.debug,"Disconnected from "+sftp.host, sshConnection); 
    ...
    sshConnection.on("ready", _ => {
        _log(ASBLOG.debug,"sftp client is connected",sshConnection);
        sshConnection.sftp((err, sftpConnection) => {
            if (err) {  // ssh connection error
                _log(ASBLOG.error,"sftp error: "+err, sshConnection); _log(ASBLOG.debug,"Disconnected from "+sftp.host, sshConnection);
            ...
            sftpConnection.readdir(path.dirname(sftp.path), (err, files) => {
                if (err || files.length == 0) {  // sftp readdir error
                    if (err) _log(ASBLOG.error,"sftp error: "+err,sshConnection); 
                    _log(ASBLOG.debug, "Disconnected from "+sftp.host, sshConnection);
                    sshConnection.end(); sftp.flow.env[routeName] = {"busy":false}; 
                    return;
                }
        ...
        _log(ASBLOG.error,"ssh connect error: "+err, sshConnection); 
        sshConnection.end(); sftp.flow.env[routeName] = {"busy":false}; 
        return;
...
function processFile(file, routeName, listener, messageContainer, sftpConnection, sshConnection, callback) {
    _log(ASBLOG.info,`Detected: ${file}`,sshConnection); 
    ...
    if (!message) {_log(ASBLOG.error,"Message creation error, throttling listener.",sshConnection); callback("Throttling error"); return;}
    sftpConnection.fastGet(file, newPath, err => {
        if (err) {_log(ASBLOG.error,`Error downloading: ${err}`,sshConnection); callback(err); return;}
        sftpConnection.rename(file, newRemotePath, err => {
            if (err) {_log(ASBLOG.error,`Error renaming remote file: ${err}`,sshConnection); callback(err); return;}
            message.env.filepath = newPath;
            message.addRouteDone(routeName);
            messageContainer.add(message);
            _log(ASBLOG.info,`Injected message with timestamp: ${message.timestamp}`,sshConnection); 
            callback();
        });
    });
}